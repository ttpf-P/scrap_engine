...
pad = Pad(char="i",
          state="float")  # The state="float" means that other Objects can be placed over it, default is "solid"
...
while True:
    if ev == "'w'":
        ...
        smap.show(
            init=True)  # The init=True option ensures, the map Map is drawn after closing the menu, even if no changes accured in the map
        smap.remap()
        ev = 0
    elif ev == "Key.space":
        shoot(player)
